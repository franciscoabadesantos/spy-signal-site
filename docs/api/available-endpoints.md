...
| Scorecard rankings | `GET /screener/rankings?reading=longTerm\|income\|shortTerm&limit=25` | `lib/picks.ts` (server-only, `unstable_cache` 300s), gated by `lib/picks-access.ts`, rendered by `app/(app)/picks/[reading]/page.tsx`. Ranks the universe by one reading in a single request; `/tickers/:ticker/scorecard` would need 686. Response is `{asOf, reading, filters, items[]}` with `capitalPerThousandIncome` present on `income` only. An unknown reading is a 422, not an empty list. Coverage (0.6) and non-company filters are applied upstream by default and are **not** exposed as options here — turning them off puts a score built from a quarter of the model at the top, and a bond ETF fourth on the income list. The `readings` are ordering, not calibrated marks |
