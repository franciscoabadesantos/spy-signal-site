...
test.describe('live ticker architecture', () => {
  ...
  test('the same equity becomes a Trade and Long-term research view', async ({ page }, testInfo) => {
    ...
    const financialsResearch = researchNav.getByRole('button', { name: 'Financials', exact: true })
    await financialsResearch.click()
    await expect(page.getByRole('menuitem', { name: 'Income Statement', exact: true })).toHaveAttribute('href', /lens=trade/)
    await page.keyboard.press('Escape')
    await expect(researchNav.getByText(/Perspective/)).toHaveCount(0)
    const moreResearch = researchNav.getByRole('button', { name: 'More', exact: true })
    await moreResearch.click()
    await expect(page.getByRole('menuitem', { name: /Ownership & Capital/ })).toBeVisible()
    await capture(page, testInfo, 'aapl-research-more-open')
    await page.keyboard.press('Escape')
    await expect(page.getByRole('menuitem', { name: /Ownership & Capital/ })).toHaveCount(0)
