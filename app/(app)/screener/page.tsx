...
function bubbleFill(direction: RowSignalDirection): string {
  if (direction === 'bullish') return 'var(--signal-bullish)'
  if (direction === 'bearish') return 'var(--signal-bearish)'
...
function biasChipActiveClass(bias: BiasFilter): string | undefined {
  if (bias === 'bullish-biased') {
    return 'signal-bg-bullish signal-bullish glow-bullish'
  }
  if (bias === 'balanced') {
    return 'border-primary/35 bg-primary/12 text-accent-text glow-key'
  }
  if (bias === 'neutral-heavy') {
    return 'signal-bg-neutral signal-neutral glow-neutral'
  }
  if (bias === 'bearish-biased') {
    return 'signal-bg-bearish signal-bearish glow-bearish'
  }
  return undefined
}
      ...
      <div className="section-gap">
                        ...
                        >
                          ...
                          <TableCell className="min-w-[126px]">
                            <div className={cn('space-y-1', isBlurredRow ? 'blur-[2px] opacity-65' : undefined)}>
                              ...
                              <div className="text-micro text-content-muted">System profile</div>
                            </div>
                          </TableCell>
                          <TableCell muted>
              ...
              {!viewer.isPro && hiddenCount > 0 ? (
                <div className="pointer-events-none absolute inset-x-0 bottom-0 flex justify-center bg-gradient-to-t from-background via-background/94 to-transparent px-6 pb-6 pt-28">
                  <PremiumPreviewCallout
                    ...
                    description={`See full system profiles, validate these signals, and interact with all ${filteredRows.length} rows.`}
                    details="See full system profile · Validate these signals · Export deeper research"
                    ctaHref={upgradeHref}
        ...
        {!viewer.isPro && hiddenCount > 0 ? (
          <PremiumPreviewCallout
            ...
            title="Unlock full signal details"
            description={`You are viewing ${previewRows.length} preview rows. Upgrade to unlock ${hiddenCount} additional tickers and full system profiles.`}
            details="See full system profile · Validate these signals"
            ctaHref={upgradeHref}
            ctaLabel={viewer.isSignedIn ? 'Upgrade Now' : 'Create Free Account'}
            openInNewTab={openUpgradeInNewTab}
          />
